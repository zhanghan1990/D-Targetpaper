...
while line:
	...
	simpleresult.append(float(words[2])*float(words[10])/1000*0.9)
	line = foffline.readline()
foffline.close()
...
while line:
	t=line.strip()
	words=t.split(' ')
	onlineresult.append(float(words[2])*float(words[10])/1000*0.8)
	line = fonline.readline()
fonline.close()
...
# 	simfactor.append(frac(simpleresult,v))
...
# ax.plot(x, simfactor,".",linewidth=3,color='k',label='simple-algorithm')
# ax.legend(loc='lower right')
# plt.ylabel('Fraction',fontsize=14,fontweight='bold')
# plt.xlabel('completion time(s)',fontsize=14,fontweight='bold')
# fig
# plt.show()
...
for v in x:
	...
	simfactor.append(frac(simpleresult,v))
ax.plot(X, offlinefactor,"-",linewidth=3,color='k',label='online-algorithm')
ax.plot(X, onlinefactor,"--",linewidth=3,color='b',label='2-approximate-algorithm')
#ax.plot(x, simfactor,".",linewidth=3,color='k',label='simple-algorithm')
ax.legend(loc='lower right')
plt.ylabel('Fraction',fontsize=14,fontweight='bold')
plt.xlabel('File access time(s)',fontsize=14,fontweight='bold')
fig
plt.show()
fig.savefig("online_offline2.pdf")
